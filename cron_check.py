"""Cron monitora cen - serce produktu.

Przechodzi po WSZYSTKICH aktywnych obserwacjach (wszystkich userow), scrapuje
kazda trase, i gdy cena spadla ponizej progu - wysyla push do wlasciciela
obserwacji (po chat_id). Przy okazji kazdy scrape zbiera dane do bazy
(ozywia analitryke historyczna).

Dwa tryby:
    python cron_check.py            # jednorazowe przejscie (do testu / Task Scheduler / launchd)
    python cron_check.py --loop     # pętla co INTERVAL_HOURS (samodzielny demon, cross-platform)

Anty-spam: alert tylko gdy cena < prog ORAZ poprzednio byla >= prog
(przekroczenie progu w dol). Stan trzymany w watched_routes.last_price.

Konfiguracja z .env:
    TELEGRAM_BOT_TOKEN   - do wysylania push
    TRAVEL_AGENT_DB      - baza
"""
...
def _offer_details(offer) -> str:
    """Szczegoly najtanszej oferty: daty, linia, numer lotu."""
    ...
    if flight:
        parts.append(f"✈️ {flight}")
    return "\n".join(parts)


def _build_alert_message(watch, offer) -> str:
    """Bogaty alert: cena + szczegoly lotu + spadek + percentyl."""
    price = offer.price
    okno = f"{watch.dep_date}–{watch.arr_date}" if watch.dep_date else "~2 mies."
    lines = [
        f"🔔 [#{watch.id}] {watch.origin} → {watch.destination} — OKAZJA!",
        f"💶 {price:.2f} EUR (prog {watch.threshold:.0f}, szukane: {okno})",
...
async def run_once() -> None:
    """Jedno przejscie po wszystkich obserwacjach."""
    ...
    try:
        for i, watch in enumerate(watches, start=1):
            ...
            # Okno dat: z obserwacji jesli podane, inaczej domyslne (dzis+7 .. dzis+60)
            ...
            if watch.mode == "always":
                # Tryb always: powiadom z aktualna cena przy kazdym sprawdzeniu
                trend = ""
                ...
                if current_price <= watch.threshold:
                    mark, rel = "✅", f"ponizej progu {watch.threshold:.0f} — okazja!"
                else:
                    mark, rel = "ℹ️", f"powyzej progu {watch.threshold:.0f}"
                msg = (
                    f"{mark} [#{watch.id}] {watch.origin} → {watch.destination}\n"
                ...
                # Tryb alert: tylko gdy przekroczenie progu w dol (anty-spam)
                crossed_down = (
...
async def run_loop() -> None:
    """Pętla: sprawdzaj co INTERVAL_HOURS. Cross-platform (nie wymaga launchd/cron)."""
